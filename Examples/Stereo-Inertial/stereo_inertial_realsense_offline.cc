...
int main(int argc, char **argv)
{
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        string pathImu = pathSeq + "/data.txt";
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            Sophus::SE3f Tcw_tose =  SLAM.TrackStereo(imLeft,imRight,tframe,vImuMeas);
            Sophus::SE3f Tcw_tose2=Tcw_tose.inverse();
            Sophus::SO3f  so3 = Tcw_tose2.so3();
            // std::cout<<Tcw_tose2.matrix().col(3)[0]<<std::endl;
            // std::cout<<Tcw_tose2.matrix().col(3)[1]<<std::endl;
            std::cout<<int(so3.matrix().eulerAngles(2,1,0)[0]*180/M_PI+90)%180<<std::endl;
